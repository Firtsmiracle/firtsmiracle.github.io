...
author      : Firtsmiracle
image       : assets/images/HTB/htb-writeup-Valentine/banner.png
category    : [ hackthebox ]
tags        : [ Information leaked, Ssl Heartbleed Explotation, Cracking Hashes, Tmux Socket File Abusing, Linux Kernel Explotation, Dirty Cow ]
---

Hola, el dia de hoy vamos a resolver la máquina `Valentine` de la plataforma de `hackthebox` correspondiente a una maquina `linux` de dificultad facil, la cual explotaremos abusando de una vulnerabilidad en el servicio `ssl` mediante la cual podremos leer registros de la memoria del servidor, obteniendo una credencial que nos permitira acceder al sistema usando una clave publica y finalmente haremos de dos formas la elevación de privilegios, una en la cual abusaremos de una `socket file` de tmux y otra donde explotaremos el kernel vulnerable a el `dirty cow` y convertirnos en el usuario `root`.
 
Asi que a darle con todo!
...
```java
❯ nmap -p- --open -sS --min-rate 5000 -vvv -n -Pn 10.10.10.79 -oG openPorts
...
Vemos que se estan ejecutando servicios web, asi que con `whatweb` vamos a tratar de identificar sus tecnologias.
...
```
Vemos que el servicio usa `php`, seguidamente vamos a abrirlo en el navegador.

![](/assets/images/HTB/htb-writeup-Valentine/vale1.PNG)

Como obervamos una imagen, vamos a descargarla para tratar si existen metadatos ocultos, para ello usaremos `exiftool`.
...
No encontramos información relevante en la imagen, asi que vamos a tratar de enumerar directorios en la web, para ello vamos a usar `wfuzz`.
...
Vemos que existe un directorio `dev`, y si lo visulizamos en el navegador este tiene como contenido dos archivos.

![](/assets/images/HTB/htb-writeup-Valentine/vale2.PNG)

Si visualizamos el archivo `hype_key`, vemos contenido en formato hexadecimal.

![](/assets/images/HTB/htb-writeup-Valentine/vale3.PNG)


Vamos a transformar el contenido para que ver a que corresponde para ello usaremos `xxd`.
...
Obtenemos una clave `id_rsa` que esta encriptada, asi que con `ssh2john` vamos a obtener un hash de la clave y con `john` trataremos de decencriptarla y obtener la contraseña.
...
Ahora si tratamos de crackear el hash con `john` haciendo uso del `rockyou.txt`, vemos que no podemos obtener la contraseña.
...
Como no encontramos nada, vamos a usar los scripts `vuln and safe` de nmap para tratar de encontrar vulnerabilidades asociadas al ssl del servicio web.
...
El escaneo nos reporta que el servicio es vulnerable a `Heartbleed`

> Heartbleed: La extensión Heartbeat para los protocolos Transport Layer Security y Datagram Transport Layer Security se propuso como un estándar en febrero del 2012 por el RFC 6520. Esto provee una forma de probar y mantener viva un enlace de comunicación segura sin la necesidad de renegociar la conexión cada vez.


La vulnerabilidad se da en la versión 1.0.1 y consiste basicamente en que permite a un atacante leer la memoria de un servidor o un cliente y permitiendo por ejemplo conseguir las claves privadas ssl.

A continuación un articulo donde puedes investigar mas a detalle:

* [https://es.wikipedia.org/wiki/Heartbleed](https://es.wikipedia.org/wiki/Heartbleed)

Para explotar la vulnerabilidad, vamos a usar el exploit de `akhld` de github.

* [https://github.com/akhld/heartbleed](https://github.com/akhld/heartbleed)


Nos descargamos el repositorio y ejecutamos el `exploit` con los parametros correspondientes.

```bash
❯ python2 heartbleed.py 10.10.10.79 -p 443  | grep -v "00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00"
Connecting...
...
El exploit funciona correctamente, vemos que se filtra cadenas de información y una de ellas corresponde a un formato en base64 `aGVhcnRibGVlZGJlbGlldmV0aGVoeXBlCg==`.

Si ahora tratamos de decodear la cadena, obtnemos un texto que parece corresponde a una contraseña.
...
Podemos tratar de conectarnos por `ssh` ya que tenemos la contraseña, pero como no sabemos el usuario al que corresponde, haciendo un poco de `guessing` podemos probar con las palabras de la contraseña que obtuvimos.


Probamos conectarnos como el usuario `hype`, sin olvidar antes dar el permiso 600 a la `id_rsa`.
...
y obtenemos acceso como el usuario `hype`, ahora podemos visualizar la primera flag `user.txt`.
...
Para elevar privilegios como el usuario podemos hacerlo de dos formas.

Primero vamos a hacer de la primera manera que es que si revisamos los archivos ocultos del directorio del usuario `hype`. Podemos observar que el `.bash_history` no es un link simbolico al `/dev/null` como de costumbre.
...
Si vemos el contenido del archivo, podemos los comandos que ejecuto el usuario entre ellos, unos de `tmux`.
...
Entre los comandos usados con `tmux`, vemos que uno de ellos hace alución a un `socket file`, que basicamente viene a ser una sesión en modo `detached`.
...
Como vemos que el usuario `root` a abierto una sesión que a almacenado a un `socket file` y este tiene como grupo asignado a `hype`, podemos asociarnos al `socket file` y nos convertiriamos en `root`.

Simplemente debemos ejecutar el comando `tmux -S /.devs/dev_sess` y ya estariamos como `root`.
...
La segunda forma de elevar privilegios seria explotar el kernel ya que si obervamos la versión que corresponde a la maquina es la `3.2.0`
...
Para ello podriamos usar el `dirty cow`, para tratar de insertar en el `/etc/passwd` un usuario nuevo con privilegios que operen como root.
...
Para ello vamos a traernos el `exploit`y vamos a ejecutarlo en la maquina victima.
...
En la maquina victima para poder ejecutarlo, si grepeamos por `gcc` nos indica como hacerlo.

```bash
hype@Valentine:/tmp$ cat exploit.c | grep gcc
//   gcc -pthread dirty.c -o dirty -lcrypt
```

Ejecutamos y nos crea un compilado con el nombre de `dirty`.

```bash
hype@Valentine:/tmp$ gcc -pthread exploit.c -o dirty -lcrypt
hype@Valentine:/tmp$ ls
_cafenv-appconfig_  dirty  exploit.c  vmware-root
```

Al ejecutar el `dirty` lo que hara sera pedirnos una contraseña, la cual asignara al usuario `firefart` que tendra privilegios como `root`.
...
Si ahora revisamos el `/etc/passwd`, vemos que nos creo el usuario exitosamente, ahora solo tenemos que migrar a el con la contraseña que introducimos.
...
Ahora nos convertimos en el usuario `firefart`, vamos al directorio personal del usuario root y podemos visualizar la segunda flag `root.txt`.
